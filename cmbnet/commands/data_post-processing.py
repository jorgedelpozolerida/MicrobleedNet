...
def get_files_metadata_from_processed(data_dir, subjects_selected=None):
    ...
    for sub in all_subjects:
        metadata_dict = read_json_to_dict(
            os.path.join(data_dir, sub, "Annotations_metadata", f"{sub}_raw.json")
        )
...
def get_subjects_metadata(args):
    ...
    id_and_preds_metadata = utils_general.load_clearml_predictions(args.predictions_dir)
...
def filter_mask_with_synthseg(
    ...
    for i in range(1, num_features + 1):
        ...
        overlap_with_non_brain = np.count_nonzero(
            ~np.isin(component_labels, list(brain_labels))
        )

        # Count overlap with brain regions
        overlap_with_brain = np.count_nonzero(
            np.isin(component_labels, list(brain_labels))
        )
        ...
        overlap_percentages = {
            label: np.mean(component_labels == label) * 100
            for label in np.unique(component_labels)
        }
        overlap_percentages_str = "\n\t\t\t".join(
            [
                f"{label} ({synthseg_labels.get(label, 'Unknown')}): {percent:.2f}%"
                for label, percent in overlap_percentages.items()
            ]
        )

        # Check for the overlap with cortex and non-brain regions
        cortex_and_non_brain_overlap = sum(
            overlap_percentages.get(label, 0)
            for label in cortex_labels | non_brain_labels
        )
        ...
        print(
            f"\t{indent}Overlaps:  {round(overlap_with_non_brain/np.sum(component)*100, 2)}-{round(overlap_with_brain/np.sum(component)*100, 2)}-{round(cortex_and_non_brain_overlap, 2)}"
        )
        print(
            f"\t{indent}Overlap percentages with label types:\n\t\t\t{overlap_percentages_str}"
        )
        ...
        if overlap_with_non_brain > 0:
            ...
            overlap_labels_str = ", ".join(
                [synthseg_labels.get(label, str(label)) for label in overlap_labels]
            )
            ...
            if overlap_with_brain > 0:
                ...
                print(
                    f"\t{indent}GOOD-pass ({result}) -> CMB partially overlaps with non-brain regions"
                )
                new_mask[component] = 1
            else:
                result = "FN-2" if component_type == "overlap" else "TN-2"
                print(
                    f"\t{indent}REMOVE ({result}) -> CMB mostly overlaps with non-brain regions"
                )
        else:
    ...
    print(all_results)
    print(
        f"OVERVIEW OF CMBs: {num_features} -----> {num_features_new} // {num_features_gt} "
    )
...
def main(args):
    ...
    studies_metadata = get_subjects_metadata(args)
    ...
    brain_labels_str = "\n".join(
        [
            f"{label}: {synthseg_labels[label]}"
            for label in BRAIN_LABELS
            if label in synthseg_labels
        ]
    )
    non_brain_labels_str = "\n".join(
        [
            f"{label}: {synthseg_labels[label]}"
            for label in non_brain_labels
            if label in synthseg_labels
        ]
    )
    ...
    for study_item in studies_metadata:
        ...
        pred_path = study_item["pred_path"]
        ...
        new_mask_path = os.path.join(
            args.processed_masks_dir, f"{studyuid}_brain_mask.nii.gz"
        )
        ...
        if not os.path.exists(new_mask_path) or (args.overwrite_newmasks):
            ...
            new_mask, res = filter_mask_with_synthseg(
                args,
                studyuid,
                pred_img_data,
                resampled_synthseg_data,
                gt_img.get_fdata(),
                brain_labels=BRAIN_LABELS,
                non_brain_labels=non_brain_labels,
