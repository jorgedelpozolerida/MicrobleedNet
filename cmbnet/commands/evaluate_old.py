...
TODO:
    - Compute evaluation from output from other script at CMB-level
...
def evaluate_study_subject_level(evaluations, subject_metadata, msg):
    """
...
def evaluate_study_CMB_level(args, subject_metadata, msg):
    ...
    results = utils_eval.get_predicted_CC_matches_and_metadata(
...
def process_study(args, subject_metadata, msg=""):
    ...
    try:
        if args.eval_mode == "per-CMB":
            ...
            evaluation_results, msg = evaluate_study_subject_level(args.evaluations, subject_metadata, msg)
        else:
            raise NotImplementedError
