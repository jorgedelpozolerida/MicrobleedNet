...
def process_MOMENIsynth_anno(mri_im: nib.Nifti1Image, com_list: list, msg: str, connectivity=6, log_level="\t\t") -> Tuple[nib.Nifti1Image, Dict, str]:
    """
    Process annotations for a Momeni dataset subject by applying region growing algorithm on CMBs based on their COM.
    """
    # Compute size threshold and maximum distance in voxels
    ...
    for i, com in enumerate(com_list):
        ...
        counter = 0
        # Iterate over combinations of parameters
        for connectivity in [6]: # only 6 connectivity
            for intensity_mode in ['point', 'running_average']:
                for diff_mode in ['relative', 'normal']:
                    if diff_mode == "relative":
                        range_temp = np.concatenate((np.arange(0, 20, 0.05), np.arange(20, 100, 1), np.arange(100, 10000, 100)))
...
def perform_MOMENIsynth_QC(subject, mris, annotations, com_list, msg):
    """
    Perform Quality Control (QC) specific to the Momeni synth dataset on MRI sequences and labels.
    ...
    for anno_sequence, anno_im in annotations.items():
        annotations_qc[anno_sequence], metadata, msg = process_MOMENIsynth_anno(anno_im, com_list, msg)
        annotations_metadata[anno_sequence] = metadata
...
def load_MOMENIsynth_data(args, subject, msg):
    ...
    # 2. Perform Quality Control and Data Cleaning
    sequences_qc, labels_qc, labels_metadata, msg = perform_MOMENIsynth_QC(subject, sequences_raw, labels_raw, com_list, msg)
